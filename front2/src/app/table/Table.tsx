...
export default function Table({
  ...
  useEffect(() => {
    ...
    if (selectAllRef.current !== null) {
      selectAllRef.current.indeterminate =
        checkedItems.length > 0 && checkedItems.length !== ids.length;
    }
  }, [ids.length, checked]);
  ...
  const handleClick = (event: any) => {
    ...
    if (target.id === "0") {
      // id for select all checkbox
      for (const id of ids) {
        temp[id] = target.checked;
      }
    } else {
  ...
  const renderTd = (item: any) => {
    return cols.map((col) => {
      ...
      if (col.callback) {
        if (col.accessor) label = col.callback(item[col.accessor]);
        else label = col.callback(item);
      } else {
  ...
  const renderCheckbox = (render: boolean, id: any) => {
    ...
    if (render) return <td key={`checkbox${id}`}>{checkbox}</td>;
  };
